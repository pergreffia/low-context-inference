...
All configuration is environment-based, parsed by `context_proxy/config.py` (pydantic-settings). Nesting uses `__` (e.g. `SERVER__MAX_BODY_BYTES`). A `.env` file in the working directory is honored; see `.env.example` for a complete skeleton.

Namespaces actually implemented: `SERVER`, `DATABASE`, `CONTEXT`, `ASSEMBLY`, `CONVERSATION`, `INFERENCE`, `QDRANT`, `RETRIEVAL`, `MEMORY`, `RESILIENCE`, `RATE_LIMIT`, `SECURITY`, `COMPACT`, `EMBEDDINGS`.

There are no `OBSERVABILITY__*` or `MEDIA__*` variables — observability is configured through `SERVER__LOG_*` and metrics are fixed-name; multimodal costs are code constants.
...
Compose-only: the published host port is `PROXY_PORT` (default `11435`) and maps to the container's fixed internal 8080.
...
Migrations run automatically at startup (`apply_migrations`); there is no separate migration command.
...
| `CONTEXT__SAFETY_MARGIN_TOKENS` | int ≥ 0 | `2048` | subtracted from limit; must be smaller than the model limit |
...
| Variable | Type | Default | Effect |
|---|---|---|---|
| `CONVERSATION__CLIENT_ID_HEADER` | str | `X-Session-ID` | fallback session header |
| `CONVERSATION__MAX_SESSION_IDENTITY_CHARS` | int > 0 | `128` | length cap on session tokens |

Identity precedence: body `conversation_id` → `X-Conversation-ID` header → session header → generated UUID.

## INFERENCE

| Variable | Type | Default | Effect |
|---|---|---|---|
| `INFERENCE__BASE_URL` | URL | `http://localhost:8000/v1` | any OpenAI-compatible inference endpoint |
...
**There is intentionally no `INFERENCE__MODEL`.** The inference model is selected by the client in the `model` request field and is forwarded unchanged. This allows one LCI instance to serve multiple models exposed by the same provider.

`GET /v1/models` is passed through to the configured inference endpoint, so clients can discover the models actually available from that provider.

## COMPACT and EMBEDDINGS

These are system-owned model endpoints and therefore retain static model configuration.
...
`COMPACT__*` is reserved for future compaction work and is not exercised by the current request flow. `EMBEDDINGS__*` backs chunk/memory embeddings for retrieval and is intentionally static system configuration.
...
The collection must be single-vector `{size: <embeddings dim>, distance: Cosine}`; an incompatible existing collection raises an explicit error and is never deleted or recreated automatically.

## RETRIEVAL

Weighted ranking: `score = semantic·w + lexical·w + recency·w + importance·w + type·w`.
...
Upstream HTTP error responses (4xx/5xx) are answers: never retried, never counted against the breaker.

## RATE_LIMIT

Two-dimension token bucket: every request consumes one client/IP token and, when `X-Conversation-ID` is present, one conversation token. A request passes only if both admit. Both dimensions share one capped table.
...
| `RATE_LIMIT__MAX_IDENTITIES` | int ≥ 1 | `10000` | live-bucket cap across both dimensions |
...
Rejected requests get OpenAI-style `429 rate_limit_error` with `Retry-After`. `/healthz`, `/readyz` and `/metrics` are exempt. Per-worker scope: with `WEB_CONCURRENCY>1` limits multiply by worker count.
...
**Fail-closed rule:** `SECURITY__MODE=production` with an empty token is a configuration error at startup — the proxy refuses to boot rather than expose administrative endpoints.

The token never appears in diagnostics output or logs.

## Interactions worth knowing

- `CONTEXT__SAFETY_MARGIN_TOKENS >= CONTEXT__MODEL_LIMIT_TOKENS` → config error.
- `SERVER__MAX_CAPTURE_BYTES` bounds streamed assistant capture; buffered responses are not size-limited by it.
- `ASSEMBLY__ENABLED=false` ignores assembly-specific scoring knobs.
- Rate-limit memory is bounded regardless of how many distinct conversation ids clients send.
...
# Client selects the model; do not configure INFERENCE__MODEL.
...
(`docker-compose.yml` sets DATABASE/QDRANT values itself; inference and embedding endpoints can point to host or service names as appropriate.)
