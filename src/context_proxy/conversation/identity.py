...
def _validated_explicit_id(raw: str, max_chars: int) -> str:
    """Validate and CANONICALIZE an explicit conversation id (hardening P2.5).

    uuid.UUID accepts braced/urn forms: the canonical `str(uuid)` form is
    returned so equivalent representations map to one conversation.
    """
    if not isinstance(raw, str) or not raw or len(raw) > max_chars:
        raise InvalidConversationId(
            f"conversation_id must be a short non-empty string "
...
def resolve_conversation_id(request: Request, payload: dict, settings=None) -> tuple[str, dict]:
    ...
    max_chars = (
        int(getattr(settings.conversation, "max_session_identity_chars", 128))
        if settings
        else 128
    )
    ...
    if raw is not None:
        return _validated_explicit_id(raw, max_chars), payload
